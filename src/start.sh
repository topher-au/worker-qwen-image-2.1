...
if [ "$SERVE_API_LOCALLY" == "true" ]; then
    python -u /comfyui/main.py ${CK_ATTENTION_ARG} ${COMFY_EXTRA_ARGS:-} --disable-auto-launch --disable-metadata --listen --verbose "${COMFY_LOG_LEVEL}" --log-stdout &
    echo $! > "$COMFY_PID_FILE"

    echo "worker-comfyui: Starting RunPod Handler"
    python -u /handler.py --rp_serve_api --rp_api_host=0.0.0.0
else
    python -u /comfyui/main.py ${CK_ATTENTION_ARG} ${COMFY_EXTRA_ARGS:-} --disable-auto-launch --disable-metadata --verbose "${COMFY_LOG_LEVEL}" --log-stdout &
    echo $! > "$COMFY_PID_FILE"
